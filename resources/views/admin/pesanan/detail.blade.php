...
<body style="background: #ebebeb">
    ...
    <a href="javascript:history.back()" style="position: absolute; z-index: 2; top: -2px; left: 10px;">
        <i class="bi bi-arrow-left-circle" style="font-size: 35px; color: black;"></i>
    </a>
    <div class="d-flex">
        <h1 class="ms-5 mt-2">Pesanan {{ \Carbon\Carbon::parse($beli->tanggal)->translatedFormat('d F Y') }}</h1>
    ...
    <div class="px-3" style="margin-top: 18px;">
        <div class="card border-0 shadow px-3">
            <div class="d-flex justify-content-start align-items-center">
                <div>
                    <img class="walah" src="../../img/akun/{{ $beli->user->img }}" alt="" style="width: 30%; border-radius: 50px;">
                </div>
                <div class="cuy">
                    <h2 style="font-size: 22px; margin-top: 3px;">{{ $beli->user->name }}</h2>
                    <p style="margin-top: -12px; color: gray;">{{ $beli->user->email }}</p>
                    <p style="margin-top: -18px;">{{ $beli->user->alamat }}</p>
                    {{-- <p>{{ now()->format('d-m-Y ') }}</p> --}}
                </div>
            </div>
            {{-- {{ $beli->tanggal }} --}}
            <div style="position: absolute; top: 0px; right: 13px;">
                <div class="alert rounded-5 text-center" style="width: 106px;padding: 1px;margin-top: 7px; font-weight: 500; {{ $beli->status == 'disiapkan' ? 'background:#ffefd4; color:#ab7b25;' : ($beli->status == 'dikirim' ? 'background:#ede1ff; color:#6c25ab;' : ($beli->status == 'selesai' ? 'background:#d5ffd4; color:#25ab2a;' : '')) }}"> {{ ucfirst($beli->status) }}</div>
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <div class="px-3" style="margin-bottom: 100px;">
        <div class="card px-3 py-2 border-0 shadow">
            ...
            <div class="d-flex justify-content-between" style="margin-top: -12px;">
                ...
                <p class="ya">Rp. {{ number_format($beli->total - 28000, 0, '.', '.') }}</p>
            </div>
            <div class="d-flex justify-content-between" style="margin-top: -12px;">
    ...
    <div class="card border-0 sticky-bottom" style="position: fixed; bottom: 0; width: 100%; border-radius: 25px 25px 0 0;">
        <div class="d-flex px-4 py-1 gap-5 align-items-center justify-content-end justify-content-lg-end ">
            <div class="align-items-center">
                <p style="color: gray; font-size: 14px; font-weight: 500;">Total</p>
                <h2 style="font-size: 18px; font-weight:700; margin-top: -16px;margin-bottom: auto; color: #1d7e9d;">Rp. {{ number_format($beli->total, 0, '.', '.') }}</h2>
            </div>
            @if ($beli->status == 'disiapkan')
                <a href="/kirim/{{ $beli->id }}" class="btn btn-primary">Kirim Sekarang</a>
            @endif
        </div>
    </div>
